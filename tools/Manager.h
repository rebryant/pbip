...
struct VariableManager {
  ...
  term get_term(string r) { // always returns term with coeff = 1! set coeff after
...
string basic_literal(string var) {
  if (var[0] == negation_token) return var.substr(1, string::npos);
  return var;
}
...
clause clause_sum(clause &a, clause &b) { // !x = 1 - x
  input_clause res;
  res.rhs = a.rhs + b.rhs;
...
struct rpn_input {
  ...
  void output() {
    for (auto x: tokens) {
      if (x.is_numeric) cout << x.d.id_value << ' ';
      else cout << x.d.op_type << ' ';
    }
  }
};
...
struct rpn_token_comp {
  bool operator()(const rpn_token a, const rpn_token b) const {
    if (a.is_numeric != b.is_numeric) return a.is_numeric;
    if (a.is_numeric) return a.d.id_value < b.d.id_value;
...
struct rpn_trie {
  ...
  struct rpn_trie_node {
    ...
    rpn_trie_node* get_child(rpn_token r) {
      if (children.find(r) != children.end()) return children[r];
  ...
  rpn_input simplify_clause(rpn_input r) {
    ...
    if (wh == -1) return r; // no simplification
...
namespace unit_propagation {
  ...
  vector<pair<int, string>> new_format_response;

  vector<clause_response> derive(vector<clause> clauses, bool new_format=false) {
    ...
    for (int i=0; i<n; i++) {
      ...
      if (clauses[i].terms.empty()) current_propagation_factor[i] = UNPROP; // if no terms can be propagated
      else current_propagation_factor[i] -= clauses[i].terms.front().coeff;
    }
    ...
    queue<pair<int, pair<bool, int>>> to_propagate; // list of things pending propagation
    ...
    vector<pair<int, pair<int, bool>>> propagations;
    int unsat_clause = -1;
	...
	auto prop = to_propagate.front(); to_propagate.pop();
	...
	for (auto occ: occs[var]) {
	  ...
	  if (!new_format) {     
	    clause_response cl;
	    cl.c = clauses[clause_id];
	    cl.hints[0] = previous_clause;
	    cl.hints[1] = source_clause_id;
	    responses.push_back(cl);
	  }
                                
	  last_version[clause_id] = next_assignable; next_assignable++;
    ...
    if (new_format) {
      string negation = "";
      negation += negation_token;
      for (auto prop: propagations) {
	new_format_response.push_back({prop.first, vm.get_string(prop.second.first, prop.second.second)});
      }
      new_format_response.push_back({unsat_clause, ""});
      return vector<clause_response>();
    }
            
    int total = n + responses.size();
    bool necessary[total];
    memset(necessary, 0, sizeof necessary);
    necessary[total - 1] = true;
    for (int i=0; i<n; i++) {
      necessary[i] = true;
    }
    ...
    for (int i=total-1; i>=n; i--) {
      if (necessary[i]) {
	int hint_a = responses[i - n].hints[0];
	int hint_b = responses[i - n].hints[1];
	necessary[hint_a] = necessary[hint_b] = true;
      }
    }
            
    int asg = 0;
    int assignments[total];
    ...
    vector<clause_response> updated;
    for (int i=n; i<total; i++) {
      if (necessary[i]) {
	clause_response r = responses[i - n];
	r.hints[0] = assignments[r.hints[0]];
	r.hints[1] = assignments[r.hints[1]];
	updated.push_back(r);
      }
    }
...
struct Manager {
  ...
  struct ClauseManager {
    ...
    int add_new_format_lookup(vector<pair<int, string>> items) {
  ...
  void add_rpn(rpn_input c) {
	...
	if (tokens[i].d.op_type == '+') {
	  ...
	  int sub_2 = stack_of_ids.back(); stack_of_ids.pop_back();
	  int sub_1 = stack_of_ids.back(); stack_of_ids.pop_back();
	  ...
	  // top one is clause
	  int sub = stack_of_ids.back(); stack_of_ids.pop_back();
	  types[sub] = CLAUSE;
  ...
  void add_derive(input_clause target_clause, bool new_format=false) {
    if(unsat_derived) {
      return;
    }
    ...
    vector<clause_response> result = unit_propagation::derive(input_clauses, new_format);

    if (new_format) {
      vector<pair<int, string>> new_res;
      for (auto &pr: unit_propagation::new_format_response) {
	new_res.push_back(make_pair(relabellings[pr.first], pr.second));
      }
    ...
    for (auto &current: result) {
      int nlabel = clauses.add_clause('A', current.c, relabellings[current.hints[0]], relabellings[current.hints[1]]);
      relabellings.push_back(nlabel);
    }
  }
  ...
  void output(const string& filename="") {
	...
	if (x.hints[0] == -2) {
	  ...
	  for (int i=0; i<t; i++) {
	    if (i + 1 == t) cout << '[' << printhint(clauses.current_new_format_items[id][i].first) <<  ']' << endl;
	    else cout << '['<< printhint(clauses.current_new_format_items[id][i].first) << ' ' << clauses.current_new_format_items[id][i].second << "] ";
	  }
	}
	else {
	...
	for (auto x: clauses.clauses) {
	  ...
	  if (x.hints[0] == -2) {
	    ...
	    int t = clauses.current_new_format_items[id].size();
	    for (int i=0; i<t; i++) {
	      if (i + 1 == t) file << '[' << printhint(clauses.current_new_format_items[id][i].first) <<  ']' << endl;
	      else file << '['<< printhint(clauses.current_new_format_items[id][i].first) << ' ' << clauses.current_new_format_items[id][i].second << "] ";
	    }
	  }
	  else {
